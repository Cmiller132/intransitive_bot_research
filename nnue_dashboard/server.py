...
class App:
    def __init__(
        ...
        self.runs_dir, self.plan, self.runner, self.token, self.control_reason = (
            runs_dir,
            plan,
            runner,
            token,
    ...
    def runs_payload(self) -> dict:
        ...
        if self.runner:
            with self.runner.lock:
                plan["reserved_names"] = [
                    j["run"] for j in self.runner.jobs if j["run"]
                ]
        payload = load_runs(self.runs_dir, plan)
        if self.runner:
            self.runner.overlay(payload["runs"])
        payload["sets_on_disk"] = sorted(
            p.parent.name
            for p in (self.runs_dir / "nnue_data").glob("*/provenance.json")
        )
        example = "models/nnue/examples/example.nnue"
        payload["networks"] = (
            [example] if self.runner and (self.runner.repo / example).is_file() else []
...
class Handler(BaseHTTPRequestHandler):
    ...
    def send_json(self, status: int, payload) -> None:
        self.send_body(
            status,
            json.dumps(finite(payload), allow_nan=False, default=str).encode(),
            "application/json",
        )
    ...
    def do_GET(self):
        ...
        try:
            if path == "/api/games":
                ...
                return self.send_json(
                    200, {"stats": games.stats(run), "live": games.live(run)}
                )
            if path == "/api/games/list":
                return self.send_json(
                    ...
                    games.listing(
                        ...
                        q.get("sort", "id"),
                        int(q.get("offset", 0)),
                        min(200, int(q.get("limit", 50))),
                    ),
                )
            if path == "/api/game":
                game = games.game(q.get("run", ""), int(q.get("id", -1)))
                return self.send_json(
                    200 if game else 404, game or {"error": "no such game"}
                )
        ...
        name = (
            "index.html"
            if path in ("/", "")
            else path.lstrip("/").removeprefix("static/")
        )
    ...
    def do_POST(self):
        ...
        if self.headers.get("X-Token") != self.app.token or (
            origin and origin.split("://", 1)[-1] != host
        ):
            return self.send_json(
                403,
                {
                    "error": "this request did not come from the dashboard page; reload it"
                },
            )
        try:
        ...
        try:
            if path == "/api/jobs/pipeline":
                ...
                job = runner.create_eval(
                    body.get("run", ""), body.get("settings") or {}
                )
                ...
                return self.send_json(
                    200, {"auto": runner.start_auto_from(body.get("run", ""))}
                )
            else:
            ...
            return self.send_json(500, {"error": repr(error)})
        return self.send_json(
            200, {"job": {k: v for k, v in job.items() if k != "commands"}}
        )
...
def ladder(text: str) -> list[int]:
    ...
    if not values or any(v <= 0 for v in values):
        raise argparse.ArgumentTypeError(
            "a comma-separated list of positive game counts, e.g. 3200,6400,12800"
        )
    return values
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Dashboard and controller for the NNUE self-play loop"
    )
    parser.add_argument(
        "--repo", help="the repository root (default: found from the working directory)"
    )
    parser.add_argument(
        "--runs",
        help="look at another runs directory; controls work only on <repo>/runs",
    )
    parser.add_argument(
        "--python",
    ...
    parser.add_argument(
        "--read-only", action="store_true", help="show the runs without the controls"
    )
    parser.add_argument(
    ...
    plan.add_argument(
        ...
        type=int,
        default=PLAN["nodes"],
        help="label nodes per move of new batches (default 50000)",
    )
    plan.add_argument(
        "--ladder",
        type=ladder,
    ...
    runs_dir = (
        Path(args.runs).resolve()
        if args.runs
        else (repo / "runs" if repo else Path("runs").resolve())
    )
    overrides = {"nodes": args.nodes, "ladder": args.ladder, "decision_sims": args.sims}
    reason = ""
    if args.read_only:
        ...
        reason = (
            f"Controls work on {repo / 'runs'} only: the trainer always writes there."
        )
    elif (
        args.host not in ("127.0.0.1", "localhost", "::1")
        and not args.allow_remote_control
    ):
    ...
    print(
        f"reading {runs_dir}"
        + (
            f"; jobs run from {repo} with {args.python}"
            if runner
            else f"; controls off: {reason}"
        )
    )
    print(
        f"serving {url}  (ctrl-c to stop{'; a running step pauses and continues from the page later' if runner else ''})"
    )
    if args.open:
        webbrowser.open(url)
