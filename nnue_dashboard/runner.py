...
STEP_WORDS = {
    "check": "Checking",
    "generate": "Generating games",
    "import": "Importing",
    "encode": "Encoding",
    "train": "Training",
    "evaluate": "Evaluating",
    "build": "Building the bot",
}
...
def default_settings(threads: int) -> dict:
    return {
        ...
        "eval": {
            "mode": "sprt",
            "sims": analysis.PLAN["decision_sims"],
            "pairs": analysis.PLAN["decision_pairs"],
            "target": analysis.PLAN["sprt_target"],
            "cap": analysis.PLAN["sprt_cap"],
        },
    }
...
class Runner:
    def __init__(
        self, repo: Path, python: str, bot: str | None = None, plan: dict | None = None
    ):
    ...
    def load(self) -> None:
        ...
        self.auto.update(
            {k: v for k, v in (data.get("auto") or {}).items() if k in self.auto}
        )
        ...
        for job in self.jobs:
            if job["state"] in ("running", "queued"):
                ...
                job["pause"] = {
                    "reason": "restart",
                    "detail": "The dashboard was restarted; continue when ready.",
                }
    ...
    def save(self) -> None:
        with self.lock:
            ...
            tmp.write_text(
                json.dumps(
                    {"jobs": self.jobs[-200:], "auto": self.auto}, indent=1, default=str
                )
            )
    ...
    # ------------------------------------------------------------------ creating jobs
    def new_job(
        self, kind: str, run: str | None, settings: dict, auto: bool = False
    ) -> dict:
        steps = (
            PIPELINE_STEPS
            if kind == "pipeline"
        ...
        job = {
            ...
            "step": steps[0],
            "steps": {
                s: {"state": "pending", "note": "", "started": None, "ended": None}
                for s in steps
            },
            "pause": None,
            "error": None,
    ...
    def validate_pipeline(self, raw: dict) -> dict:
        ...
        for k, v in (raw or {}).items():
            if k == "eval":
                s["eval"].update(
                    {kk: vv for kk, vv in (v or {}).items() if kk in s["eval"]}
                )
        ...
        if not NAME.fullmatch(name) or name.startswith("nnue_") or ".replaced_" in name:
            raise ValueError(
                "a name of letters, digits, '.', '_' and '-' that does not start with nnue_"
            )
        s["name"] = name
        ...
        s["extra"] = [
            {"set": str(e["set"]), "share": float(e.get("share", 1.0))}
            for e in s["extra"]
            if str(e.get("set", "")) and str(e["set"]) != f"selfplay_{name}"
        ]
        ...
        if busy:
            raise ValueError(
                f"{name} already has an unfinished job; continue or cancel it first"
            )
        return s
    ...
    def validate_eval(e: dict) -> dict:
        ...
        if e["mode"] == "sprt" and (
            not 0.5 < e["target"] < 1 or e["cap"] < 128 or e["cap"] % 16
        ):
            raise ValueError(
                "the sequential test needs a target between .5 and 1 and a cap that is a multiple of 16, at least 128"
            )
        return e
        ...
        with self.lock:
            ...
            if action == "pause":
                if state == "running":
                    ...
                    job["state"], job["pause"] = (
                        "paused",
                        {"reason": "user", "detail": "Paused before it started."},
                    )
                ...
                job.update(
                    state="queued", queued_at=time.time(), pause=None, error=None
                )
                ...
                if job["step"] != "train" or state != "running":
                    raise ValueError(
                        "pausing after an epoch applies while training runs"
                    )
                ...
                if job["kind"] != "pipeline" or state not in (
                    "paused",
                    "failed",
                    "done",
                ):
                    raise ValueError(
                        "training restarts from a paused, failed or finished pipeline"
                    )
                changes = (
                    self.validate_pipeline(
                        {**job["settings"], **payload, "name": job["run"]}
                    )
                    if state == "done"
                    else None
                )
                if (
                    state == "done"
                ):  # a finished job stays as the record; the retraining is a new job
                    return self.new_job(
                        "pipeline", job["run"], {**changes, "restart_training": True}
                    )
                for key in (
                    "epochs",
                    "passes",
                ...
                if state not in TERMINAL and state != "paused":
                    raise ValueError(
                        "only finished, failed, cancelled or paused jobs can be removed"
                    )
        ...
        while not self.closing:
            with self.lock:
                if self.current is None:
                    queued = sorted(
                        (j for j in self.jobs if j["state"] == "queued"),
    ...
    def execute(self, job: dict) -> None:
        ...
        try:
            if job["kind"] == "build":
                ...
                start = (
                    PIPELINE_STEPS.index(job["step"])
                    if job["step"] in PIPELINE_STEPS
                    else 0
                )
                for name in PIPELINE_STEPS[start:]:
                    if (
                        job["steps"][name]["state"] in ("done", "skipped")
                        and name != "check"
                    ):
                        continue
            ...
            with self.lock:
                job.update(
                    state="paused",
                    pause={"reason": p.reason, "detail": p.detail},
                    updated=time.time(),
                )
        except Interrupted:
            with self.lock:
                ...
                if why == "cancel":
                    ...
                    job.update(
                        ...
                        pause={
                            "reason": "user",
                            "detail": "Paused during "
                            + STEP_WORDS.get(job["step"], job["step"]).lower()
        ...
        if job["state"] == "done" and not self.closing:
            try:
                ...
                with self.lock:
                    self.auto.update(
                        enabled=False,
                        message=f"The automatic mode stopped on an error: {e}",
                    )
                    self.save()
    ...
    def run_step(self, job: dict, name: str, fn) -> None:
        ...
        with self.lock:
            ...
            step.update(
                state="running",
                started=step["started"] or time.time(),
                note=step["note"] if step["state"] == "paused" else "",
            )
            self.save()
        try:
            note = fn(job)
        ...
        with self.lock:
            ...
            step.update(
                ...
                ended=time.time(),
                note=note[0] if skipped else (note or ""),
            )
            self.save()
    ...
    def command(self, job: dict, step: str, cmd: list[str], stdout=None) -> str:
        ...
        if self.interrupts.get(job["id"]):
            raise Interrupted()
        shown = " ".join(
            c if re.fullmatch(r"[\w@%+=:,./-]+", c) else json.dumps(c) for c in cmd
        )
        with self.lock:
        ...
        with log.open("a", encoding="utf-8") as out:
            ...
            if stdout:
                readers.append(
                    threading.Thread(
                        target=pump, args=(proc.stderr, False), daemon=True
                    )
                )
                readers[-1].start()
        ...
        if code != 0:
            last = next(
                (l for l in reversed(tail) if l.strip() and not l.startswith("$ ")), ""
            )
    ...
    def probe(self, *args: str) -> dict:
        try:
                ...
                [self.python, str(self.probe_file), *args],
                cwd=self.repo,
                capture_output=True,
                text=True,
                timeout=900,
            ...
            raise StepFailed(
                f"the training interpreter {self.python} could not be run: {e}"
            )
    ...
    def step_check(self, job: dict):
        ...
        if not (self.repo / self.bot).is_file():
            raise StepFailed(
                f"{self.bot} is missing: build it first (the Build button, or cargo build --release -p cli)"
            )
        ...
        version = (
            9
            if str(s["version"]) == "9"
            or (s["version"] == "auto" and net["version"] == 9)
            else 8
        )
        ...
        if version == 9 and not caps["version9"]:
            raise StepFailed(
                "format 9 needs a newer trainer (git pull, then reinstall models/nnue/py)"
            )
        ...
        missing = (
            [
                n
                for n, v in self.probe("sets", *[e["set"] for e in s["extra"]]).items()
                if not v["exists"]
            ]
            if s["extra"]
            else []
        ...
        if missing:
            raise StepFailed(
                "earlier batches not found under runs/nnue_data: " + ", ".join(missing)
            )
        foreign = self.runs / s["name"]
        if (
            foreign.is_dir()
            and any(foreign.iterdir())
            and not (foreign / "config.json").is_file()
        ):
            raise StepFailed(
                f"runs/{s['name']} exists but is not a training run; choose another name"
            )
        manifest = analysis.read_json(
            self.runs / "nnue_selfplay" / s["name"] / "manifest.json"
        )
        seed = s["seed"]
        if manifest:
            if manifest.get("model_sha256") != net["sha256"] and not manifest.get(
                "complete"
            ):
                raise StepFailed(
                    f"runs/nnue_selfplay/{s['name']} is an unfinished batch from another network; use a new name"
                )
            if seed is None:
    ...
    def step_generate(self, job: dict):
        ...
        if manifest and manifest.get("complete"):
            return (
                f"{sum(sh['counts']['games'] for sh in manifest.get('shards', [])):,} games already played",
            )
        m = (manifest or {}).get("settings")
        if m:  # a batch on disk keeps its own settings, or it cannot resume
            games, nodes, threads, hash_mib, seed = (
                m["games"],
                m["nodes"],
                m["threads"],
                m["hash_mib"],
            ...
            mpv = [
                ...
                str(m.get("multipv", 1)),
                "--multipv-margin",
                str(m.get("multipv_margin", 60)),
                "--multipv-prob",
                str(m.get("multipv_prob", 50)),
                "--multipv-nodes",
                str(m.get("multipv_nodes", 25)),
                "--pv-labels",
                str(m.get("pv_labels", 0)),
            ]
        else:
            games, nodes, threads, hash_mib, seed = (
                s["games"],
        ...
        if not manifest.get("complete"):
            raise StepFailed(
                "the generator stopped before the batch was complete; continue to resume it"
            )
    ...
    def step_import(self, job: dict):
        ...
        rows = (
            analysis.read_json(self.runs / "nnue_data" / own / "provenance.json") or {}
        ).get("rows", 0)
        return f"{rows:,} rows"
    ...
    def step_encode(self, job: dict):
        ...
        for name in todo:
            self.command(
                job, "encode", [self.python, "-m", "nnue.data", "encode", name]
            )
    ...
    def train_args(self, job: dict) -> tuple[list[str], dict]:
        ...
        steps = (
            ...
            if s.get("steps")
                ...
                10, math.ceil(rows * float(s["passes"]) / (s["batch"] * s["epochs"]))
            )
        )
        warmup = (
            int(s["warmup"])
            if s.get("warmup")
            else max(1, min(200, steps * s["epochs"] // 10))
        )
        if s["ema"] == "auto":
        ...
        if ema:
            args += ["--ema", ema]
        return args, {"rows": rows, "steps": steps, "warmup": warmup, "ema": ema}
    ...
    def step_train(self, job: dict):
        ...
        if s.get("end_training_early") and (out / "best.nnue").is_file():
            ...
            return (f"ended by hand after epoch {done} of {s['epochs']}",)
        ...
        if state["state"] == "mismatch":
            if not s.get("restart_training"):
                raise StepFailed(
                    f"runs/{s['name']} was trained with other settings ({state.get('why', '')}). "
            ...
            with self.lock:
                ...
                job["steps"]["train"]["note"] = (
                    f"the old run moved to runs/{aside.name}"
                )
        ...
        while done < s["epochs"]:
            ...
            if now <= done:
                raise StepFailed(
                    f"the trainer finished without writing epoch {done + 1}"
                )
            done = now
            with self.lock:
                wants = s.get("pause_every_epoch") or job.pop(
                    "pause_after_epoch", False
                )
            if wants and done < s["epochs"]:
                raise Paused(
                    "epoch",
                    f"Paused after epoch {done} of {s['epochs']}; best.nnue holds the best epoch so far.",
                )
    ...
    def step_evaluate(self, job: dict):
        ...
        reference = (
            e.get("reference") or job["settings"].get("start") or config.get("init")
        )
        ...
        threads = int(
            job["settings"].get("threads") or e.get("threads") or self.threads
        )
        ...
        if anchor:  # measured against another network: kept apart from the decision (analysis reads eval_vs_*)
            slug = re.sub(
                r"[^A-Za-z0-9]+",
                "_",
                Path(reference).parent.name
                if Path(reference).name == "best.nnue"
        ...
        if e["mode"] == "sprt":
            ...
            file = f"eval_{versus}{early}sprt_{k}.json"
        else:
        ...
        cmd = [
            ...
            "eval",
            "--candidate",
            f"nnue:runs/{run}/best.nnue?hash={hash_mib}",
            "--reference",
            f"nnue:{reference}?hash={hash_mib}",
            "--sims",
            str(e["sims"]),
            "--threads",
            str(threads),
        ...
        return (
            f"score {score:.3f} over {games:,} games"
            + (f", test {stop}" if stop else "")
            + f" ({file})"
        )
    ...
    def after(self, job: dict) -> None:
        if (
            not self.auto["enabled"]
            or job["kind"] not in ("pipeline", "eval")
            or not job["run"]
        ):
            return
        if self.busy():
    ...
    def follow(self, run_name: str) -> dict | None:
        ...
        nxt = d.get("next")
        if not nxt:
            return self.stop_auto(
                f"{run_name}: {d['title'].lower()}; nothing to do automatically"
            )
        bad = [
            c
            for c in run["checks"]
            if c["status"] == "bad" and c["group"] in ("Training", "Data")
        ]
        if self.auto["pause_on_problems"] and bad and d["verdict"] != "reeval":
            return self.stop_auto(
                f"{run_name} has training or data problems ({', '.join(c['label'] for c in bad)}); "
        ...
        with self.lock:
            if d["verdict"] == "adopt":
                if self.auto["adopted"] >= self.auto["max_generations"]:
                    ...
                    return self.stop_auto(
                        f"{n} new generation{'s' if n != 1 else ''} done, the limit; {run_name} is adopted and waits"
                    )
                self.auto["adopted"] += 1
            if nxt["kind"] == "pipeline":
                if self.auto["batches"] >= self.auto["max_batches"]:
                    ...
                    return self.stop_auto(
                        f"{n} batch{'es' if n != 1 else ''} played, the limit"
                    )
                self.auto["batches"] += 1
        try:
            if nxt["kind"] == "pipeline":
                settings = {
                    **nxt["settings"],
                    ...
                    "pause_every_epoch": self.auto["pause_every_epoch"],
                }
                ...
                paused = next(
                    (
                        ...
                        if j["run"] == run_name and j["state"] in ("paused", "failed")
                    ),
                    None,
                )
                if not paused:
                    return self.stop_auto(
                        f"{run_name} stopped outside the dashboard; continue it by hand"
                    )
    ...
    def progress(self, job: dict) -> dict:
        ...
        out = {
            "step": step,
            "words": STEP_WORDS.get(step, step or ""),
            "done": None,
            "total": None,
            "unit": "",
            "eta_seconds": None,
        ...
        if step == "generate" and run:
            ...
            if gen:
                out.update(
                    ...
                    total=gen["games_target"],
                    unit="games",
                    in_flight=gen["in_flight"],
                )
                if (
                    gen["games_per_hour"]
                    and gen["games_target"]
                    and job["state"] == "running"
                ):
                    out["eta_seconds"] = (
                        (gen["games_target"] - gen["games_done"])
                        / gen["games_per_hour"]
                        * 3600
                    )
            ...
            e = s["eval"]
            out.update(
                done=self.eval_games.get(job["id"], 0),
                unit="games",
                total=None if e["mode"] == "sprt" else 2 * e["pairs"],
                cap=2 * e["cap"] if e["mode"] == "sprt" else None,
            )
        return out
    ...
    def snapshot(self) -> dict:
        with self.lock:
            ...
            for job in self.jobs[-60:]:
                ...
                view["tail"] = (
                    list(self.tails.get(job["id"], []))[-40:]
                    if job["state"] in ("running", "paused", "failed")
                    else []
                )
                if (
                    not view["tail"]
                    and job["state"] in ("paused", "failed")
                    and job["step"]
                ):
                    ...
                    try:
                        view["tail"] = [
                            l[:400]
                            for l in log.read_text(errors="replace").splitlines()[-40:]
                        ]
                    except OSError:
                        pass
                jobs.append(view)
            return {
                ...
                "defaults": default_settings(self.threads),
                "bot_exists": (self.repo / self.bot).is_file(),
                "python": self.python,
                "repo": str(self.repo),
            }
    ...
    def overlay(self, runs: list[dict]) -> None:
        ...
        for run in runs:
            ...
            state = {"running": "running", "queued": "queued", "paused": "paused"}[
                job["state"]
            ]
            detail = p["words"]
            if p.get("total"):
            ...
            if job["state"] in ("running", "queued") or (
                job["state"] == "paused" and job["step"] != "evaluate"
            ):
                run["decision"] = {
                    **run["decision"],
                    "verdict": "running",
                    "title": {
                        "running": STEP_WORDS.get(job["step"], "Running"),
                        "queued": "Queued",
                        "paused": "Paused",
                    }[job["state"]],
                    "summary": detail.capitalize()
                    + ("" if detail.endswith(".") else "."),
                    "reasons": ["The decision waits for this job's evaluation."],
                    "next": None,
                }
