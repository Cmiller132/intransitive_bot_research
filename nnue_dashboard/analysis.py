...
PLAN = {
    ...
    "reading_pairs": 100,
    "sprt_target": 0.52,  # C1's upper hypothesis, about +14 Elo
    "sprt_cap": 3008,
...
DEPTH_NOTE = (
    "Measured against the start at the evaluation budget only. In the research loop, gains of +19 to +39 Elo "
    "at 40k nodes read between -3 and +23 on the arena at 200k: deeper search compresses them, often to about half."
)

METRIC_HELP = {
    "objective": (
        "Validation error that picks best.nnue: the share-weighted mean of each set's selection mse.",
        "Lower. It should keep falling or flatten late; a minimum in epoch 0-2 followed by a rise means memorising.",
    ),
    "loss": (
        "Training loss (value loss + symmetry_weight x consistency), averaged over the epoch's steps.",
        "Falls steadily. Only meaningful next to objective: loss falling while objective rises is overfitting.",
    ),
    "value": (
        "The value part of the training loss (tanh mse plus the small logit term).",
        "Falls with loss.",
    ),
    "consistency": (
        "Squared disagreement between two random board symmetries of the same position.",
        "Falls towards zero; the network should score a position the same from every symmetry.",
    ),
    "lr": (
        "Learning rate at the end of the epoch: linear warmup, cosine down to 15 % of the peak.",
        "The recipe's shape; a flat or odd curve means the schedule was overridden.",
    ),
    "seconds": (
        "Wall time of the epoch, including validation.",
        "Stable; spikes are contention on the machine.",
    ),
    "best": ("Lowest objective so far.", "A staircase that keeps stepping down."),
    "mse": (
        "Validation mean squared error against the search labels, all six symmetries.",
        "Lower. Compare epochs within one run; the level depends on the data.",
    ),
    "mae": (
        "Validation mean absolute error.",
        "Lower; less swayed by a few hard positions than mse.",
    ),
    "bias": (
        "Mean signed error: positive means the network is more optimistic than the search.",
        "Close to zero, within about +-0.005.",
    ),
    "symmetry_range_mean": (
        "Average spread of the six symmetric evaluations of one position.",
        "Lower and falling: symmetric positions should evaluate alike.",
    ),
    "symmetry_range_p95": (
        "95th percentile of that spread: the worst symmetric disagreements.",
        "Lower and falling.",
    ),
    "rows": (
        "Validation rows in this slice.",
        "Small slices (a few hundred rows) are noisy; read their curves loosely.",
    ),
}
STRATA_HELP = {
...
def count_lines(path: Path, needle: bytes | None = None) -> int:
    ...
    return (
        data.count(b"\n")
        if needle is None
        else sum(1 for line in data.split(b"\n") if needle in line)
    )
...
def strength(raw: dict) -> dict:
    ...
    if counts and sum(counts) > 1:
        ...
        sd_pair = math.sqrt(
            sum(c * (v - mean) ** 2 for c, v in zip(counts, values)) / (n - 1)
        )
    ...
    out = {
        ...
        "elo_se": (score_to_elo(hi_s) - score_to_elo(lo_s)) / (2 * 1.96)
        if se
        else None,
        "nelo": gain / (sd_pair * math.sqrt(2)) * 800 / math.log(10)
        if sd_pair > 0
        else None,
        "los": normal_cdf(gain / se)
        if se
        else (1.0 if gain > 0 else 0.0 if gain < 0 else 0.5),
        ...
        "pairs_needed": math.ceil(n * (1.96 * se / gain) ** 2)
        if se and gain > 0
        else None,
    }
    return out
...
def eval_summary(path: Path) -> dict | None:
    ...
    if seq:
        ...
        if stop == "accept":
            verdict = "better"
        elif stop == "inconclusive" and lo_s > 0.5:
            verdict = "better_small"
        elif stop in ("reject", "inconclusive"):
            verdict = "worse" if hi_s < 0.5 else "null"
        else:
            verdict = "running" if stop == "running" else "invalid"
        grade = (
            "decision" if stop in ("accept", "reject", "inconclusive") else "partial"
        )
    else:
        stop, target, cap = None, None, None
        grade = (
            "decision"
            if sims >= PLAN["decision_sims"] and pairs >= PLAN["decision_pairs"]
            else "reading"
        )
    ...
    return {
        ...
        **st,
        ...
        "bootstrap_score_interval": [0.5 + boot[0] / 2, 0.5 + boot[1] / 2]
        if boot
        else None,
...
def generation_summary(runs_dir: Path, name: str) -> dict | None:
    ...
    pending = (
        len(list((folder / "pending").glob("*.json")))
        if (folder / "pending").is_dir()
        else 0
    )
    active = (
        list((folder / "active").glob("*.jsonl"))
        if (folder / "active").is_dir()
        else []
    )
    touched = max(
        [p.stat().st_mtime for p in [folder / "manifest.json", *active] if p.exists()],
        default=0,
    )
    search_hours = totals.get("search_ns", 0) / 3.6e12
    wall_rate = (
        games / (search_hours / threads) if search_hours else None
    )  # games per hour with every thread busy
    return {
        ...
        "alternative_share": totals.get("alternative_roots", 0) / totals["roots"]
        if totals.get("roots")
        else 0.0,
...
def dataset_summary(runs_dir: Path, name: str) -> dict | None:
    ...
    line = max(
        0, int(counts.get("line_rows", 0)) - int(counts.get("line_duplicates", 0))
    )
    ...
    return {
        ...
        "quiet": {
            "kept": quiet.get("roots_kept"),
            "skipped": quiet.get("skipped") or {},
        }
        if quiet
        else None,
...
def run_names(runs_dir: Path) -> list[str]:
    ...
    if runs_dir.is_dir():
        for d in runs_dir.iterdir():
            if (
                d.is_dir()
                and not d.name.startswith(("nnue_", "."))
                and ".replaced_" not in d.name
            ):
                names.add(d.name)
...
def load_run(runs_dir: Path, name: str) -> dict | None:
    ...
    found = [
        e for e in (eval_summary(p) for p in sorted(directory.glob("eval*.json"))) if e
    ]
    ...
    anchors = [
        e
        for e in found
        if e["reference_path"] and start and e["reference_path"] != start
    ]
    evals = [e for e in found if e not in anchors]
    best_mtime = (
        (directory / "best.nnue").stat().st_mtime
        if (directory / "best.nnue").is_file()
        else None
    )
    for e in (
        found
    ):  # an evaluation from before the network last changed describes an older network
    ...
    own = next(
        (
            m["set"]
            for m in mixture
            if re.fullmatch(rf"selfplay_{re.escape(name)}(_quiet.*)?", m["set"])
        ),
        f"selfplay_{name}",
    )
    ...
    primary = (
        max(current, key=lambda e: (rank[e["grade"]], e["mtime"])) if current else None
    )
    files = (
        [p for p in directory.iterdir() if p.is_file()] if directory.is_dir() else []
    )
    ...
    return {
        ...
        or (generation or {}).get("model_sha256"),
        "best": {
            k: meta.get(k)
            for k in (
                "epoch",
        ...
        "started": min(
            (
                p.stat().st_mtime
                for p in (directory / "config.json", directory / "log.csv")
                if p.is_file()
            ),
            default=(generation or {}).get("touched", 0),
        ),
        "mtime": max(
            [p.stat().st_mtime for p in files] + [(generation or {}).get("touched", 0)],
            default=0,
        ),
    }
...
def training_stats(run: dict) -> dict:
    ...
    sets = [m["set"] for m in run["mixture"]] or sorted(
        {c.split(".")[0] for c in log["columns"] if "." in c}
    )
    ...
    for s in sets:
        ...
        if len(vals) >= 2 and isinstance(at_best, (int, float)):
            drift[s] = {
                ...
                "at_best": at_best,
                "last": vals[-1],
                "change_to_best": (at_best - vals[0]) / vals[0],
            }
    tail = rows[best_i:]
    ...
    diverging = (
        best_i < len(rows) - 2
        and len(loss_tail) >= 2
        and loss_tail[-1] < loss_tail[0]
        and (last_v - best_v) / best_v > SWEET_SPOTS["objective_rise_warn"]
    )
    q = max(2, len(valid) // 4)
    recent = [v for _, v in valid[-q:]]
    slope = (
        (recent[-1] - recent[0]) / (len(recent) - 1) / recent[0]
        if len(recent) > 1
        else 0.0
    )
    seconds = [
        r.get("seconds") for r in rows if isinstance(r.get("seconds"), (int, float))
    ]
    return {
...
def run_status(run: dict, runs_dir: Path, now: float) -> dict:
    ...
    if gen and not gen["complete"]:
        ...
        eta = (
            (target - done) / gen["games_per_hour"] * 3600
            if gen["games_per_hour"] and target
            else None
        )
        live = now - gen["touched"] < stale
        return {
            "state": "generating" if live else "stopped",
            "step": "generate",
            "fraction": done / target if target else None,
            "eta_seconds": eta if live else None,
            "detail": f"{done:,} of {target:,} games"
            + (f", {gen['in_flight']} in flight" if live and gen["in_flight"] else ""),
        }
    if not run["config"]:
        own = run["datasets"].get(run["own_set"]) if run.get("datasets") else None
        touched = max(
            (p.stat().st_mtime for p in logs.glob("*.log")),
            default=gen["touched"] if gen else 0,
        )
        live = now - touched < stale
        return {
            "state": "preparing" if live else "stopped",
            "step": "import",
            "fraction": None,
            "eta_seconds": None,
            "detail": "importing and encoding the batch"
            if live and not own
            else "the batch is ready for training",
        }
    if tr and not tr["finished"] and not run["eval"]:
        live = now - run["log"]["mtime"] < max(
            stale, 3 * (tr["seconds_per_epoch"] or 0)
        )
        left = tr["epochs_planned"] - tr["epochs_done"]
        return {
            "state": "training" if live else "stopped",
            "step": "train",
            "fraction": tr["epochs_done"] / tr["epochs_planned"],
            "eta_seconds": left * tr["seconds_per_epoch"]
            if live and tr["seconds_per_epoch"]
            else None,
    ...
    if not tr and run["config"] and not run["eval"]:
        return {
            ...
            "fraction": 0.0,
            "eta_seconds": None,
            "detail": "loading the data",
        }
    stream = logs / "evaluate.jsonl"
    if run["has_net"] and (
        not run["eval"]
        or (stream.is_file() and stream.stat().st_mtime > run["eval"]["mtime"])
    ):
    ...
    return {
        "state": "done" if run["eval"] else "idle",
        "step": None,
        "fraction": None,
        "eta_seconds": None,
        "detail": "",
...
def load_runs(runs_dir: Path, plan: dict | None = None) -> dict:
    ...
    by_sha = {
        r["best"]["sha256"]: r["name"]
        for r in runs
        if r["best"] and r["best"].get("sha256")
    }
    ...
    for run in runs:
        run["children"] = sorted(
            (r["name"] for r in runs if r["parent"] == run["name"]),
            key=lambda n: by_name[n]["started"],
        )
        key = run["init_sha256"] or run["init"]
        run["siblings"] = [
            r["name"]
            for r in runs
            if r is not run and key and (r["init_sha256"] or r["init"]) == key
        ]
        run["training"] = training_stats(run)
        run["datasets"] = {
            m["set"]: dataset_summary(runs_dir, m["set"]) for m in run["mixture"]
        }
    ...
    return {
        ...
        "runs": runs,
        "roots": sorted(
            (r["name"] for r in runs if not r["parent"]),
            key=lambda n: by_name[n]["started"],
        ),
        "plan": plan,
        "ladder": [
            {
                "generation": i,
                "games": g,
                "rows": g * plan["rows_per_game"],
                "why": LADDER_WHY[min(i, len(LADDER_WHY) - 1)]
                if len(plan["ladder"]) == len(PLAN["ladder"])
                else "",
            }
...
def lineage(run: dict, by_name: dict) -> list[dict]:
    ...
    for k, node in enumerate(chain):
        ...
        direct = [
            ...
            if not a["stale"] and a["reference_path"] == root_start
        ]
        if (
            direct
        ):  # measured straight against the line's first network: no stacking error
            best = max(direct, key=lambda a: (a["pairs"], a["mtime"]))
            step["direct"] = {
                "elo": best["elo"],
                "elo_interval": best["elo_interval"],
                "pairs": best["pairs"],
                "sims": best.get("sims"),
                "file": best["file"],
            }
...
def checks(run: dict, by_name: dict, plan: dict, on_disk: dict) -> list[Check]:
    ...
    if ev:
        ...
        if ev["kind"] == "sequential":
            words = {
                "accept": f"accepted {ev['sprt_target']} over .50",
                "reject": f"rejected a gain of {ev['sprt_target']}",
                "inconclusive": f"no decision within {ev['sprt_cap']:,} pairs",
            ...
            out.append(
                Check(
                    "Evaluation",
                    "Sequential test",
                    words.get(ev["stop"], str(ev["stop"])),
                    "accept, or inconclusive with the lower bound above .50",
                    status,
                    ...
                    if isinstance(ev["llr"], (int, float))
                    else "The test's report carries no LLR.",
                )
            )
        else:
            out.append(
            ...
            out.append(
                Check(
                    ...
                    "Opening pairs",
                    f"{pairs:,}",
                    f"{plan['decision_pairs']} for a fixed-count decision",
        ...
        out.append(
            Check(
                "Evaluation",
                "Resolution",
                f"±{ev['resolution_elo']:.0f} Elo" if ev["resolution_elo"] else "–",
        ...
        if b and ev["kind"] == "fixed":
            ...
            out.append(
                Check(
                    ...
                    "Bootstrap interval",
                    f"{b[0]:.3f} to {b[1]:.3f}",
                    "agrees with the interval",
                    "ok" if agree else "warn",
                    "Resampled pairs; it should tell the same story.",
                )
            )
        out.append(
            Check(
                "Evaluation",
                "Forfeits",
                str(ev.get("forfeits", 0)),
                "0",
                "ok" if not ev.get("forfeits") else "bad",
                "A forfeit is a crash or an illegal move, not a result.",
            )
        )
    ...
    if tr:
        ...
        early = (
            tr["best_epoch"] <= s["early_best_epochs"]
            and tr["epochs_done"] > s["early_best_epochs"] + 2
        )
        out.append(
            Check(
                "Training",
                "Best epoch",
                f"{tr['best_epoch']} of {tr['epochs_planned'] - 1}",
                "second half of the run",
                "info"
                if running
                else "bad"
                if early and tr["rise_after_best"] > s["objective_rise_warn"]
                else "ok"
                if late
                else "warn",
        ...
        out.append(
            Check(
                "Training",
                "Objective rise after best",
                f"{rise:+.2%}",
                f"under {s['objective_rise_warn']:.1%}",
                "ok"
                if rise <= s["objective_rise_warn"]
                else "warn"
                if rise <= s["objective_rise_bad"]
                else "bad",
        ...
        out.append(
            Check(
                "Training",
                "Loss against objective",
                "diverging" if tr["diverging"] else "moving together",
                "moving together",
                "bad" if tr["diverging"] else "ok",
        ...
        for name, d in tr["drift"].items():
            ...
            out.append(
                Check(
                    "Training",
                    f"Older batch {name.removeprefix('selfplay_')}",
                    f"mse {ch:+.2%}",
        ...
        worst_bias = max(
            (
                abs(v)
                for v in tr["bias_at_best"].values()
        ...
        if worst_bias is not None:
            out.append(
                Check(
                    "Training",
                    "Worst bias at the best epoch",
                    f"{worst_bias:.4f}",
                    f"under {s['bias_warn']}",
                    "ok"
                    if worst_bias < s["bias_warn"]
                    else "warn"
                    if worst_bias < s["bias_bad"]
                    else "bad",
    ...
    if cfg:
        ...
        out.append(
            Check(
                "Training",
                "Weight averaging",
                f"decay {ema:g} (about {window:,.0f} steps)" if ema else "off",
                "about one epoch of steps",
                "ok"
                if ema and steps and 0.5 * steps <= window <= 2 * steps
                else "info",
        ...
        if cfg.get("version"):
            out.append(
                Check(
                    "Training",
                    "Network format",
                    f"format {cfg['version']}",
                    "8 (9 is untested for strength)",
    ...
    if sets:
        out.append(
            Check(
                "Data",
                "Batches in the mixture",
                str(len(sets)),
                "every batch of the line",
                "ok"
                if len(sets) >= 2 or run["depth"] == 0 and not run["siblings"]
                else "warn",
        ...
        if rows:
            out.append(
                Check(
                    "Data",
                    "Root rows in the mixture",
                    f"{rows / 1e6:.2f} M",
                    "grows with every batch",
                    "info",
        ...
        missing = sorted(
            n
            for n in line_sets(run, by_name, on_disk)
            if n not in kept
            and n != run["own_set"]
        ...
        out.append(
            Check(
                "Data",
                "Earlier batches kept",
                "all"
                if not missing
                else "missing "
                + ", ".join(n.removeprefix("selfplay_") for n in missing),
                "every batch from this line",
                "ok" if not missing else "warn",
                "Keep every batch; the next command passes them all in EXTRA_DATA.",
            )
        )
    for sib in run["siblings"]:
        ...
        if ours is not None and ours == theirs:
            out.append(
                Check(
                    "Data",
                    "Seed against siblings",
                    str(ours),
                    f"different from {sib}",
                    "bad",
    ...
    if gen:
        total = (
            f"{gen['games_target']:,}" if isinstance(gen["games_target"], int) else "?"
        )
        out.append(
            Check(
                "Data",
                "Self-play batch",
                f"{gen['games_done']:,} of {total} games",
                "complete",
                "ok" if gen["complete"] else "info",
                "An unfinished batch resumes when the same command runs again.",
            )
        )
        if isinstance(gen["games_target"], int):
            ...
            out.append(
                Check(
                    ...
                    "Batch size",
                    f"{g:,} games",
                    f"{want:,} or more at generation {run['depth']}",
        ...
        if gen.get("multipv", 1) > 1 or gen.get("pv_labels"):
            out.append(
                Check(
                    "Data",
                    "Generation extras",
                    f"multipv {gen.get('multipv', 1)}, PV labels {gen.get('pv_labels', 0)}",
                    "optional",
                    "info",
                    "The loop's settings, adopted without their own A/B.",
                )
            )
        out.append(
            Check(
    ...
    if own and own.get("quiet"):
        ...
        out.append(
            Check(
                "Data",
                "Quiet filter",
                f"{dropped:,} roots dropped, {q['kept']:,} kept",
                "optional",
                "info",
...
def next_generation(name: str, taken: set[str], depth: int) -> str:
    ...
    candidate = (
        ORDINALS[ORDINALS.index(base) + 1]
        if base in ORDINALS[:-1]
        else f"{base}_g{depth}"
    )
...
def next_pipeline(
    run: dict, start: str, name: str, games: int, extra: dict[str, float], plan: dict
) -> dict:
    ...
    return {
        ...
        "settings": {
            ...
            "epochs": cfg.get("epochs") or 20,
            "batch": cfg.get("batch") or 8192,
            "lr": cfg.get("lr") or 0.0001,
            "device": cfg.get("device") or "cpu",
            "version": "9" if cfg.get("version") == 9 else "auto",
            "quiet": bool((run["datasets"].get(run["own_set"]) or {}).get("quiet")),
            "multipv": (run["generation"] or {}).get("multipv", 1),
            "eval": {
                "mode": "sprt",
                "sims": plan["decision_sims"],
                "pairs": plan["decision_pairs"],
                "target": plan["sprt_target"],
                "cap": plan["sprt_cap"],
            },
        },
    }
...
def quickstart_command(
    ...
    parts = [
        ...
        f"NODES={plan['nodes']}",
        "SPRT=1",
        f"SIMS={plan['decision_sims']}",
        *settings_env(run),
    ]
    if extra:
        parts.append(
            'EXTRA_DATA="' + " ".join(f"{n}:{w:g}" for n, w in extra.items()) + '"'
        )
...
def resume_command(run: dict, by_name: dict) -> str | None:
    ...
    if cfg:
        ...
        parts += [
            f"SEED={cfg['seed']}",
            f"EPOCHS={cfg['epochs']}",
            f"STEPS={cfg['steps_per_epoch']}",
            f"WARMUP={cfg['warmup_steps']}",
            f"EMA={cfg.get('ema') or 0:g}",
        ...
        if extra:
            parts.append(
                'EXTRA_DATA="'
                + " ".join(f"{m['set']}:{m['share']:g}" for m in extra)
                + '"'
            )
...
def sprt_eval_command(run: dict, plan: dict) -> str:
    ev = run["eval"] or {}
    ...
    journal = f"runs/{run['name']}/sprt_s{plan['decision_sims']}.jsonl"
    resume = (
        " --resume" if f"sprt_s{plan['decision_sims']}.jsonl" in run["journals"] else ""
    )
    return (
        f'target/release/bot eval --candidate "{cand}" --reference "{ref}" \\\n  --sims {plan["decision_sims"]} '
        f"--threads {threads} --seed {seed} --sprt {journal}{resume} \\\n  > runs/{run['name']}/eval_sprt_s{plan['decision_sims']}.json"
    )
...
def decide(run: dict, by_name: dict, plan: dict, on_disk: dict) -> dict:
    ...
    taken = (
        set(by_name)
        | {n.removeprefix("selfplay_") for n in on_disk}
        | set(plan.get("reserved_names", []))
    )
    start = run["init"] or "<start.nnue>"
    reasons: list[str] = []
    unhealthy = [
        c for c in run["checks"] if c["status"] == "bad" and c["group"] != "Evaluation"
    ]

    gen = run["generation"] or {}
    this_batch = {
        "games": gen.get("games_target") or ladder_games(plan, run["depth"]),
        "generation": run["depth"],
        "base": ladder_games(plan, run["depth"]),
        "nulls": 0,
        "current": True,
        "rows": (gen.get("games_target") or ladder_games(plan, run["depth"]))
        * plan["rows_per_game"],
        "why": "",
    }
    ...
    def result(
        ...
        return {
            ...
            "command": command,
            "command_label": label,
            "extra_commands": extra or [],
            "batch": batch or this_batch,
            "next": next_step,
        }
    ...
    def retry_batch() -> dict:
        ...
        why = (
            f"Generation {run['depth']} plans {base:,} games; {nulls} batch{'es' if nulls != 1 else ''} from this start "
            f"showed no gain, so the next one is {games // base}x the plan (the loop doubles after each null, up to 4x)."
        )
        return {
            "games": games,
            "rows": games * plan["rows_per_game"],
            "generation": run["depth"],
            "base": base,
            "nulls": nulls,
            "why": why,
    ...
    if state in ("generating", "preparing", "training", "evaluating"):
        words = {
            "generating": "Generating games",
            "preparing": "Importing the batch",
            "training": "Training",
            "evaluating": "Evaluating",
        }
        reasons.append(
            "The decision waits for the evaluation at the end of the quickstart run."
        )
        return result("running", f"{words[state]}", status["detail"].capitalize() + ".")
    if state == "stopped":
        reasons.append(
            "Nothing has been written for a while: the run was interrupted. The same command continues it; "
        ...
        if run["config"] or run["generation"]:
            ...
            resume = {
                ...
                "run": run["name"],
                "settings": {
                    "start": cfg.get("init") or gen.get("init"),
                    "name": run["name"],
                    "games": gen.get("games_target")
                    or ladder_games(plan, run["depth"]),
                    "nodes": gen.get("nodes") or plan["nodes"],
                    "extra": [m for m in run["mixture"] if m["set"] != run["own_set"]],
                    "seed": cfg.get("seed", gen.get("seed")),
                    "epochs": cfg.get("epochs") or 20,
    ...
    if not ev:
        if run["has_net"]:
            reasons.append(
                f"Nothing is adopted or dropped without an evaluation: the sequential test at "
                f"{plan['decision_sims'] * 2500:,} nodes decides."
            )
            return result(
                "evaluate",
                "Evaluate before deciding",
                "best.nnue exists but no current evaluation of it.",
                sprt_eval_command(run, plan),
        ...
        return result(
            ...
            "This run has no network and no evaluation.",
        )

    lo_s, hi_s = ev["score_interval"]
    budget = f"{ev.get('sims', 0) * 2500:,} nodes"
    reasons.append(
        f"Score {ev['score']:.3f} over {ev['pairs']:,} pairs at {budget}: {ev['elo']:+.0f} Elo "
        f"({ev['elo_interval'][0]:+.0f} to {ev['elo_interval'][1]:+.0f})."
    )

    if ev["verdict"] in ("running", "invalid"):
        reasons.append(
            "The sequential test did not finish. Rerunning it with --resume continues the same journal."
            if ev["verdict"] == "running"
            else "The sequential test is invalid (a forfeit, a ply cap or an "
            "interruption inside a game); run it again with a new journal."
        )
        return result(
            "reeval",
            "Finish the sequential test",
            "The evaluation stopped before its own rule decided.",
            sprt_eval_command(run, plan),
            "Continue the sequential test",
            next_step=sprt_next,
        )

    if ev["kind"] == "fixed" and ev["grade"] == "reading" and ev["verdict"] != "worse":
        why = (
            "Wins seen at a shallower budget were often gone at 40k nodes in the research loop"
            if (ev.get("sims") or 0) < plan["decision_sims"]
            else f"{ev['pairs']} pairs cannot tell a +14 Elo gain from noise"
        )
        reasons.append(
            f"This is a reading ({ev['pairs']} pairs at {budget}), not a decision. {why}; the sequential test "
            "settles it in 128 to 3,008 pairs, stopping early when the answer is clear."
        )
        return result(
            "reeval",
            "Confirm with the sequential test",
            "Test the network at the decision budget before "
            "spending hours on the next batch.",
            sprt_eval_command(run, plan),
            "Run the sequential test",
            next_step=sprt_next,
        )
    ...
    if ev["verdict"] in ("better", "better_small"):
        if ev["verdict"] == "better":
            reasons.append(
                f"The sequential test accepted a gain of {ev['sprt_target']} over .50."
                if ev["kind"] == "sequential"
                else "The interval's lower bound is above .50 at the decision budget."
            )
        else:
            reasons.append(
        ...
        if ev["kind"] == "sequential" and ev["verdict"] == "better":
            reasons.append(
                "The Elo after an early accept is optimistic; the next generation's test is the real check."
            )
        if unhealthy:
            reasons.append(
                "Some training checks failed; the win stands, but look at them before the next batch."
            )
        depth = run["depth"] + 1
        ...
        batch = {
            ...
            "base": games,
            "nulls": 0,
            "why": f"Generation {depth} of this line plans {games:,} games"
            + (
                f", {LADDER_WHY[min(depth, len(LADDER_WHY) - 1)]}."
                if plan["ladder"] == PLAN["ladder"]
                else "."
            ),
        }
        ...
        return result(
            "adopt",
            f"Adopt {run['name']} as the next start",
            f"Continue self-play from runs/{run['name']}/best.nnue with every batch kept.",
            quickstart_command(
                run, f"runs/{run['name']}/best.nnue", nxt, games, extra, plan
            ),
            "Next generation",
            batch=batch,
            next_step=next_pipeline(
                run, f"runs/{run['name']}/best.nnue", nxt, games, extra, plan
            ),
        )
    ...
    if ev["verdict"] == "worse":
        reasons.append(
            "The upper bound is below .50: this network is worse than its start."
        )
        if unhealthy:
            reasons.extend(
                f"{c['label']}: {c['value']}, looking for {c['target']}. {c['note']}"
                for c in unhealthy
            )
            reasons.append(
                "Fix the training before blaming the data: PASSES at 4 and every earlier batch in the mixture."
            )
        else:
            reasons.append(
                "Training looks healthy, so the batch did not help on its own; keep it and add another."
            )
        reasons.append(batch["why"])
        return result(
            "abort",
            f"Drop {run['name']}, go back to {start_label(run)}",
            "Keep its games in the mixture and play another batch from the previous network.",
            quickstart_command(run, start, name, batch["games"], extra, plan),
            "Another batch from the previous start",
            batch=batch,
            next_step=next_pipeline(run, start, name, batch["games"], extra, plan),
        )

    reasons.append(
        f"The test rejected a gain of {ev['sprt_target']}: no gain large enough to see."
        if ev["kind"] == "sequential" and ev["stop"] == "reject"
        else "No decision within the cap and the interval still straddles .50."
        if ev["kind"] == "sequential"
        else "The interval straddles .50 at the decision budget."
    )
    if ev["pairs_needed"] and ev["pairs_needed"] > ev["pairs"]:
        reasons.append(
            f"If {ev['elo']:+.0f} Elo were real, about {ev['pairs_needed']:,} pairs would show it; "
