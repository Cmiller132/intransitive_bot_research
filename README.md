...
```
cargo xtask check       # the one check: fmt, clippy, tests, engine wheel, ruff, pytest (CPU)
...
and triton; `pip install -e "models/sq/py[dev]"` installs the training package
with pytest, ruff and maturin. xtask/README.md describes each task and the
one-time setup of the Linux build environment.
