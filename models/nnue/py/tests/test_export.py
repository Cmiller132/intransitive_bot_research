...
from nnue.features import BUCKETS, CONTEXTS, FORMAT8, PIECE_ROWS, context, feature_ids, piece_bucket
...
@pytest.mark.parametrize("hidden,buckets,version", [(32, 1, 8), (64, BUCKETS, 8), (512, 1, 6), (32, BUCKETS, 6)])
def test_integer_file_reproduces_the_quantised_forward(tmp_path, hidden, buckets, version):
    torch.set_num_threads(1)
    net = trained_like(NNUE(hidden, buckets, version), 5)
    boards, since, clock = positions(21, 96)
    path = tmp_path / "net.nnue"
    info = export(net, path)
    assert info["bytes"] == file_size(hidden, buckets, version) == path.stat().st_size
...
def test_sizes_match_the_contract():
    assert file_size(512, 1, 6) == 1_064_172
    assert file_size(512, 1, 8) == 14_003_436
    assert HEADER.size == 36


def test_conversion_copies_every_piece_row_into_all_contexts_and_keeps_the_values(tmp_path):
    net = trained_like(NNUE(32, 1, 6), 13)
    six, eight = tmp_path / "six.nnue", tmp_path / "eight.nnue"
    export(net, six)
    info = convert(six, eight)
    assert info["version"] == 8 and info["bytes"] == file_size(32, 1, 8) == eight.stat().st_size
    ...
    with torch.no_grad():
        ...
        after = wide(ids, bucket, qat=True)
...
def test_widened_hidden_evaluates_identically_with_distinct_new_columns(tmp_path):
    ...
    assert wide.hidden == 64 and wide.buckets == net.buckets and wide.version == net.version
    boards, since, clock = positions(4, 40)
    a = evaluate(export(net, tmp_path / "a.nnue") and tmp_path / "a.nnue", boards, since, clock)
    b = evaluate(export(wide, tmp_path / "b.nnue") and tmp_path / "b.nnue", boards, since, clock)
    assert np.array_equal(a, b)
