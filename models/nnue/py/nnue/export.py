"""The integer file the Rust crate loads (RPSNNUE1 version 6), an independent
NumPy evaluator over its bytes, and the conversion of the prototype's
version 3 files (DESIGN items 4 and 10).

Layout, little endian: magic `RPSNNUE1`; u32 version 6; u32 features 1004;
...
from .features import BUCKETS, CLOCK_BUCKETS, FEATURES, PIECE_ROWS, SLOTS, feature_ids, piece_bucket
...
def file_size(hidden: int, buckets: int) -> int:
    return (
        HEADER.size
        + 2 * hidden
        + 2 * FEATURES * hidden
...
def export(model: NNUE, path: Path, metadata: dict | None = None) -> dict:
    ...
    h = model.hidden
    raw = HEADER.pack(MAGIC, VERSION, FEATURES, h, QA, QB, EVAL_SCALE, model.buckets)
    ...
    assert len(raw) == file_size(h, model.buckets)
    path = Path(path)
...
def read(path: Path) -> dict:
    ...
    if magic != MAGIC or version != VERSION or features != FEATURES or (qa, qb) != (QA, QB):
        raise ValueError("not an RPSNNUE1 version 6 file with the expected constants")
    if buckets not in (1, BUCKETS) or len(data) != file_size(hidden, buckets):
...
def integer_eval(path: Path, board: np.ndarray, since_capture: np.ndarray, clock: np.ndarray) -> np.ndarray:
    ...
    weights = np.concatenate([net["weights"], np.zeros((1, hidden), dtype=np.int64)])
...
def load(path: Path) -> NNUE:
    ...
    net = read(path)
    model = NNUE(net["hidden"], net["buckets"])
    with torch.no_grad():
        model.bias.copy_(torch.from_numpy(net["bias"] / QA).float())
        model.embedding.weight[:FEATURES].copy_(torch.from_numpy(net["weights"] / QA).float())
        model.embedding.weight[FEATURES].zero_()
...
def convert_v3(source: Path, target: Path, metadata: dict | None = None) -> dict:
    ...
    raw = HEADER.pack(MAGIC, VERSION, FEATURES, hidden, QA, QB, scale, 1)
    raw += bias.tobytes()
    ...
    assert len(raw) == file_size(hidden, 1)
    target = Path(target)
